...
class BotFeedbackAdapter(val context: Context, var datalist:List<String>) : RecyclerView.Adapter<ResponseViewHolder1>() {

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ResponseViewHolder1 {
        val itemLayoutBinding = BotitemlayoutBinding.inflate(LayoutInflater.from(parent.context),parent,false)
        return ResponseViewHolder1(itemLayoutBinding)
    }

    override fun onBindViewHolder(holder: ResponseViewHolder1, position: Int) {
...
class ResponseViewHolder1(val itemLayoutBinding: BotitemlayoutBinding) : RecyclerView.ViewHolder(itemLayoutBinding.root)