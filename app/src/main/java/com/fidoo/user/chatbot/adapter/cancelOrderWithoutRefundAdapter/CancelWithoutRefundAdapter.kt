...
class CancelWithoutRefundAdapter(val context: Context, var datalist:List<String>) : RecyclerView.Adapter<ResponseViewHolder3>() {

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ResponseViewHolder3 {
        val itemLayoutBinding = BotitemlayoutBinding.inflate(LayoutInflater.from(parent.context),parent,false)
        return ResponseViewHolder3(itemLayoutBinding)
    }

    override fun onBindViewHolder(holder: ResponseViewHolder3, position: Int) {
...
class ResponseViewHolder3(val itemLayoutBinding: BotitemlayoutBinding) : RecyclerView.ViewHolder(itemLayoutBinding.root)