...
class BtnYesNoAdapter(val context: Context, var datalist:List<String>) : RecyclerView.Adapter<ResponseViewHolder4>() {

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ResponseViewHolder4 {
        val itemLayoutBinding = BotitemlayoutBinding.inflate(LayoutInflater.from(parent.context),parent,false)
        return ResponseViewHolder4(itemLayoutBinding)
    }

    override fun onBindViewHolder(holder: ResponseViewHolder4, position: Int) {
...
class ResponseViewHolder4(val itemLayoutBinding: BotitemlayoutBinding) : RecyclerView.ViewHolder(itemLayoutBinding.root)