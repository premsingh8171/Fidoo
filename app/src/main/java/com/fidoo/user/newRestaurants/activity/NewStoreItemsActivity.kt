...
import kotlinx.android.synthetic.main.activity_new_store_items.backIcon
import kotlinx.android.synthetic.main.activity_new_store_items.linear_progress_indicator
...
class NewStoreItemsActivity :
	...
	var categoryHeaderAdapter: CategoryHeaderAdapter?=null
		...
		if (isNetworkConnected) {
			...
			if (SessionTwiclo(this).isLoggedIn) {
				...
				viewmodel?.getStoreDetailsApiNew(
					...
					nonveg_str,
					cat_id,
					contains_egg, cartCount.toString()
				)
			} else {
				viewmodel?.getStoreDetailsApiNew(
					"",
