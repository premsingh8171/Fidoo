...
//            }
             ...
             if (!storeData.offers.isNullOrEmpty()) {
                 tv_coupon.text =
                     "Flat " + storeData.offers[0].discount + " % OFF On Total Cart Value"
                         ...
                         for (i in storeData.subcategory.indices) {
                             ...
                             for (j in 0 until storeData.subcategory[i].product.size) {
                                 ...
                                 if (productData.customize_item.size != 0) {
                                     ...
                                     val regex = "\\[|\\]"
                                     lastCustomized_str =
                                         lastCustomized.replace(regex.toRegex(), "")
                                     product_customize_id = "1"
                                 }
...
//            )
     ...
     private fun getvegitems() {
         ...
         CoroutineScope(Dispatchers.IO).launch {
             ...
             isVegApplied = restaurantProductsDatabase!!.resProductsDaoAccess()!!
                 .getAllVegProduct("0", totalItem.toString())
...
//                }
      ...
      inner class Task : AsyncTask<Void, Void, ArrayList<StoreItemProductsEntity>>() {


          override fun onPreExecute() {
              viewmodel?.newStoreDetailsRes?.observe(this@NewDBStoreItemsActivity, Observer { storeData ->
                  linear_progress_indicator.visibility = View.GONE
                  Log.d("getStoreDetailsApi__", Gson().toJson(storeData))
...
//                toast.show()
          ...
          override fun doInBackground(vararg p0: Void?): ArrayList<StoreItemProductsEntity> {
